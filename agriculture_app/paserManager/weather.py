...
def getHtml(url):
    try:
        #url = "http://www.weather.com.cn/weather/" + citycode + ".shtml"  # 构造网址
        head = {}
     ...
     # }
        ...
        html = response.read()#.decode('utf-8')  # bytes 解码 得到 str
    except error.HTTPError as e:
        print(e.code)
    except error.URLError as e:
        print(e.reason)  # html = html.decode('GBK')
...
def getWeather(html):
    ...
    for link in soup.find_all(id="hidden_title"):
        ...
        # print(link)
    pattern_weather = re.compile(r'<.*? value="(.*?)"/>')
    for href in href_list:
        result = re.match(pattern_weather, href)
        # print(type(result.groups()))
        if result:
            link_href = result.groups()[0]
            #print("%s" % link_href)
            final.append(link_href)
    ...
    for day in li:  # 对每个标签中的内容进行遍历
        ...
        # print(type(inf)) #<class 'bs4.element.ResultSet'>
        ...
        temp.append(temperature_high+'/'+temperature_lower)
        #temp.append(temperature_high)
        win = inf[2].find('i').string
        temp.append(win)
        final.append(temp)  # 将temp添加到final中
    return final

if __name__=="__main__":
    cityname = input("你要查询的城市：")
    citycode_r = getCity(cityname)
    print(citycode_r)
    url = "http://www.weather.com.cn/weather/" + citycode_r + ".shtml"  # 构造网址
    while citycode_r:
        html=getHtml(url)
        result=getWeather(html)
        for i in result:
            print(i)        #打印天气情况
        time.sleep(120)
